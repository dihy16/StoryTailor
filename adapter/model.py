...
class MSAdapter(torch.nn.Module):
    def __init__(self, unet, image_proj_model, adapter_modules=None, ckpt_path=None,
        ...
        if ckpt_path is not None:
            # self.load_from_checkpoint_v2(ckpt_path)
    ...
    def load_from_checkpoint_v2(self, ckpt_path: str):
        if os.path.isdir(ckpt_path):
            ckpt_path = os.path.join(ckpt_path, "ms_adapter.bin")

        # helper: safe checksum for possibly-empty parameter lists
        def _safe_sum_params(mod: torch.nn.Module) -> torch.Tensor:
            params = [p for p in mod.parameters()]
            if len(params) == 0:
                return torch.tensor(0.0)
            return torch.sum(torch.stack([torch.sum(p) for p in params]))

        # Calculate original checksums
        orig_ip_proj_sum = _safe_sum_params(self.image_proj_model)
        orig_adapter_sum = _safe_sum_params(self.adapter_modules)

        state_dict = torch.load(ckpt_path, map_location="cpu")

        # 1) image_proj_model must match strictly
        self.image_proj_model.load_state_dict(state_dict["image_proj"], strict=True)

        # 2) adapter_modules: v2 may contain teleport processors (no params) or different keys
        #    so load non-strictly to avoid crashing. (You already load cross-attn weights elsewhere.)
        try:
            self.adapter_modules.load_state_dict(state_dict["ms_adapter"], strict=False)
        except Exception as e:
            print(f"[v2][warn] adapter_modules load skipped due to mismatch: {e}")

        # dummy tokens
        self.load_state_dict({"dummy_image_tokens": state_dict["dummy_image_tokens"]}, strict=False)

        # Calculate new checksums
        new_ip_proj_sum = _safe_sum_params(self.image_proj_model)
        new_adapter_sum = _safe_sum_params(self.adapter_modules)

        # Verify image proj changed (important)
        assert orig_ip_proj_sum != new_ip_proj_sum, "Weights of image_proj_model did not change!"

        # Adapter may legitimately not change / be empty in v2; only warn
        if orig_adapter_sum == 0.0 and new_adapter_sum == 0.0:
            print("[v2][info] adapter_modules has no parameters; checksum check skipped.")
        elif orig_adapter_sum == new_adapter_sum:
            print("[v2][warn] adapter_modules checksum did not change (could be expected in v2).")
    ...
    def get_image_embeds(self, processed_images, image_encoder=None, image_proj_type="linear",
        ...
        processed_images = processed_images.view(-1, processed_images.shape[-3], processed_images.shape[-2],
                                                 processed_images.shape[-1])  # (bsz * rn, C, H, W)
    ...
    def generate(self, pipe, pil_images=None, processed_images=None, prompt=None, negative_prompt=None, scale=1.0,
                 ...
                 boxes=None, phrases=None, drop_grounding_tokens=None, phrase_idxes=None,action=None,
                 frame_prompt_list=None, window_length=None, id_prompt=None,
                 eot_idxes=None, height=1024, width=1024, subject_scales=None, mask_threshold=None, start_step=5, unet_controller = None, inherit_region = None,
                 **kwargs):
        ...
        if action is not None:
            with torch.no_grad():
                ...
                out = pipe.text_encoder(action_input)
                action_embeds = getattr(out, "last_hidden_state", out[0]).to(self.device, dtype=weight_dtype)  # [1,S,C]
                action_mask = (action_input != pipe.tokenizer.pad_token_id).to(self.device)  # [1,S]

                Bp = bsz * num_samples
                cond_embeds = action_embeds.expand(Bp, -1, -1).contiguous()  # [B',S,C]
                cond_mask = action_mask.expand(Bp, -1).contiguous()  # [B',S]
                uncond_embeds = torch.zeros_like(cond_embeds)  # [B',S,C]
                uncond_mask = torch.zeros_like(cond_mask)  # [B',S]

                action_embeds = torch.cat([uncond_embeds, cond_embeds], dim=0)  # [2B',S,C]
                action_mask = torch.cat([uncond_mask, cond_mask], dim=0)  # [2B',S]

                cross_attention_kwargs['action_embeds'] = action_embeds
                cross_attention_kwargs['action_mask'] = action_mask
                cross_attention_kwargs["action_map_save_dir"] = "./action_map/action_maps_token_svr"
                cross_attention_kwargs["action_map_upsample_to"] = (1024 ,1024)
                cross_attention_kwargs["action_map_use_focus_heads"] = True
        ...
        with torch.inference_mode():
            ...
            image_prompt_embeds = image_prompt_embeds.to("cpu")
