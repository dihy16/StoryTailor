...
class ActionEmbedder(nn.Module):
    def __init__(self, text_encoder: CLIPTextModel, tokenizer: CLIPTokenizer, target_dim: int = 1664, freeze_encoder: bool = True):
        super().__init__()
        self.text_encoder = text_encoder
        self.tokenizer = tokenizer
        self.proj = nn.Linear(text_encoder.config.hidden_size, target_dim)

        if freeze_encoder is True:
            for param in self.text_encoder.parameters():
                param.requires_grad = False

    def forward(self, action_phrases):
        # action_phrases: List[str], e.g. ["chasing", "hugging"]
        device = next(self.parameters()).device
        inputs = self.tokenizer(action_phrases, padding=True, return_tensors="pt").to(device)
        with torch.no_grad():
            outputs = self.text_encoder(**inputs)
            text_embed = outputs.pooler_output  # [B, D_text]
        image_like_embed = self.proj(text_embed)  # map to image latent space
        return image_like_embed
...
class ImageProjDummyModel(torch.nn.Module):
    ...
    def forward(self, image_embeds, magi_image_embeds=None):
        ...
        if self.use_magi and magi_image_embeds is not None:
            ...
            magi_image_embeds = magi_image_embeds.view(bsz, max_num_ips * self.clip_extra_context_tokens,
