...
class StableDiffusionXLPipeline(
    ...
    def __init__(
        ...
        # init noise
        # cache for last frame's x0 latents (diffusion latent space, before VAE unscale)
        self._last_x0_latents = None
        # optionally remember last latent shape to guard mismatches
        self._last_x0_shape = None
        self._pl_state = {}
                ...
                if unet_controller is not None and unet_controller.frame_prompt_express is not None:
                    ...
                    if unet_controller.Prompt_embeds_mode == 'svr':
                        ...
                        for movement in frame_prompt_suppress:
                            utils.swr_single_prompt_embeds(movement, input_prompt_embeds[0], prompt[0],
                                                           unet_controller.tokenizer, alpha=alpha_weaken,
                                                           beta=beta_weaken, zero_eot=True)
                        utils.swr_single_prompt_embeds(frame_prompt_express, input_prompt_embeds[0], prompt[0],
                                                       unet_controller.tokenizer, alpha=alpha_strengthen,
                        ...
                        if unet_controller.Prompt_embeds_mode == 'svr':
                            for movement in frame_prompt_suppress:
                                utils.swr_single_prompt_embeds(movement, input_prompt_embeds[index], prompt[index],
                                                               unet_controller.tokenizer, alpha=alpha_weaken,
                                                               beta=beta_weaken)
                            utils.swr_single_prompt_embeds(frame_prompt_express, input_prompt_embeds[index],
                                                           prompt[index], unet_controller.tokenizer,
                                                           alpha=alpha_strengthen, beta=beta_strengthen)
    ...
    def __call__(
            ...
            unet_controller: Optional[UNetController] = None,
            # 新参数
            use_last_x0_as_init: bool = False,
            view_jitter: float = 0.0,  # 例如 0.01~0.05，小幅视角扰动强度
            view_jitter_mode: str = "affine",  # 先留一个模式位，默认仿射
            reset_prev_latents: bool = False,  # 置 True 可在一帧（或第一帧）手动清空缓存
            cache_prev_on_cpu: bool = True,  # 省显存可放 CPU
            inherit_strength: float = 0.0,
            # 局部继承相关
            inherit_region: Optional[str] = "tl",  # "tl"|"tr"|"bl"|"br"；不传=全图
            inherit_rect: Optional[Tuple[float, float, float, float]] = None,  # (x0,y0,x1,y1) in [0,1]
            inherit_softness: float = 0.12,  # 软边（相对最短边）
            inherit_fit_global: bool = False,  # ★ 把上一帧“整图”缩放后贴到 region
            inherit_keep_aspect: bool = True,  # 保比例（letterbox）
            inherit_exclude_boxes: Optional[torch.Tensor] = None,  # (B,N,4) 归一化 xyxy（当前帧主体）
            inherit_halo_px: int = 8,  # 主体膨胀像素（latent 空间）
            subject_softness_px: int = 2,  # 主体边缘羽化
            inherit_lowpass_px: int = 6,  # ★ 低通滤波（只保背景）
            inherit_t_ratio: float = 0.70,
            # —— 文本侧跨帧（仅混合文本嵌入，不动 latent）——
            carry_text: float = 0.35,  # token 级（prompt_embeds）混合权重
            carry_pooled: float = 0.60,  # pooled 级（pooled_prompt_embeds）混合权重
            reset_text_cache: bool = False,  # 该帧是否清空上一帧文本缓存
        ...
        # optional: reset cached latents (e.g., at the first frame)
        if reset_prev_latents:
            self._last_x0_latents = None
            self._last_x0_shape = None
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                unet_controller.num_inference_steps = self.num_timesteps
        ...
        if not output_type == "latent":
            ...
            try:
                to_store = latents.detach()
                if cache_prev_on_cpu:
                    to_store = to_store.to("cpu", non_blocking=True).float()
                else:
                    to_store = to_store.clone()
                self._last_x0_latents = to_store
                self._last_x0_shape = tuple(latents.shape)
        ...
        unet_controller.end_frame()

        try:
            x0_for_next = latents.detach()
            if cache_prev_on_cpu:
                self._pl_state["prev_x0_cpu"] = x0_for_next.to("cpu", non_blocking=True).float()
                self._pl_state["prev_x0_gpu"] = None
            else:
                self._pl_state["prev_x0_gpu"] = x0_for_next.clone()
                self._pl_state["prev_x0_cpu"] = None
        except Exception:
            pass
